    ...
    <main class="bg-[#f7f2ed] px-6 py-10 lg:px-10">
        <section class="mx-auto max-w-7xl">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <p class="text-xs font-bold uppercase tracking-[0.35em] text-[#b7a69a]">Keranjang</p>
                    <h1 class="mt-2 text-3xl font-black text-[#6f5a4c]">Produk Pilihan Saya</h1>
                    <p class="mt-2 text-sm text-[#9a8b81]">Atur jumlah produk sebelum lanjut ke checkout.</p>
                </div>
            ...
            @if ($cartItems->isEmpty())
                <div class="mt-8 rounded-lg border border-dashed border-[#d8c8bb] bg-white px-6 py-16 text-center shadow-sm">
                    <h2 class="text-xl font-black text-[#6f5a4c]">Keranjang masih kosong</h2>
                    <p class="mt-2 text-sm text-[#9a8b81]">Pilih produk organik SIRACAS yang ingin Anda pesan.</p>
                    <a href="{{ route('customer.product.index') }}"
                        class="mt-6 inline-flex h-11 items-center justify-center rounded-lg bg-[#9e836f] px-5 text-sm font-bold text-white transition hover:bg-[#8a725f]">
                        Lihat Produk
                    </a>
                </div>
            @else
                <div class="mt-8 grid gap-6 lg:grid-cols-[1fr_320px] lg:items-start">
                    <div class="overflow-hidden rounded-lg border border-[#e2d6cc] bg-white shadow-sm">
                        <div class="hidden grid-cols-[1fr_210px_170px_80px] gap-4 border-b border-[#eadfd7] px-5 py-4 text-xs font-bold uppercase tracking-[0.22em] text-[#aa9d94] md:grid">
                            <span>Produk</span>
                            <span>Quantity</span>
                            ...
                            @foreach ($cartItems as $item)
                                ...
                                <article class="grid gap-4 px-5 py-5 md:grid-cols-[1fr_210px_170px_80px] md:items-center">
                                    <div class="flex gap-4">
                                        ...
                                        <div class="min-w-0">
                                            <h2 class="line-clamp-2 text-base font-black text-[#5f4f45]">{{ $product->nama_produk }}</h2>
                                            <p class="mt-1 text-sm font-bold text-[#b37323]">
                                                Rp{{ number_format((float) $item->harga_saat_dimasukkan, 0, ',', '.') }}
                                            </p>
                                            <p class="mt-1 text-xs font-semibold text-[#9a8b81]">Stok: {{ $product->stok }}</p>
                                        </div>
                                    </div>
                                    ...
                                    <form action="{{ route('cart.items.update', $item) }}" method="POST"
                                        ...
                                        @method('PATCH')
                                        <button type="button"
                                            class="cart-quantity-minus inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-[#d8c9bc] bg-white text-lg font-black text-[#6f5a4c] transition hover:bg-[#f7f1eb]"
                                            aria-label="Kurangi quantity {{ $product->nama_produk }}">
                                            -
                                        </button>
                                        <input type="number" name="quantity" min="0" max="{{ $product->stok }}"
                                            value="{{ old('quantity', $item->quantity) }}"
                                            class="cart-quantity-input h-10 w-20 rounded-lg border border-[#e1d5cb] bg-[#fbf8f5] px-3 text-center text-sm font-bold text-[#5d5048] outline-none transition focus:border-[#a6866d] focus:bg-white focus:ring-2 focus:ring-[#eadfd7]">
                                        <button type="button"
                                            class="cart-quantity-plus inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-[#d8c9bc] bg-white text-lg font-black text-[#6f5a4c] transition hover:bg-[#f7f1eb]"
                                    ...
                                    <div>
                                        <p class="text-xs font-semibold uppercase text-[#aa9d94] md:hidden">Total Harga</p>
                    ...
                    <aside class="rounded-lg border border-[#e2d6cc] bg-white p-5 shadow-sm">
                        <p class="text-xs font-bold uppercase tracking-[0.3em] text-[#b7a69a]">Ringkasan</p>
                        <div class="mt-4 space-y-3 text-sm">
                            <div class="flex items-center justify-between gap-4 text-[#8f8178]">
                                <span>Total item</span>
                                <span class="font-bold text-[#5f4f45]">{{ $cartItems->sum('quantity') }}</span>
                            </div>
                            <div class="flex items-center justify-between gap-4 text-[#8f8178]">
                                <span>Subtotal</span>
                                <span class="text-lg font-black text-[#b37323]">
